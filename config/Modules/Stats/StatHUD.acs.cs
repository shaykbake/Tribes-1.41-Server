	...
	for ( %team = -1; %team <= 1; %team++ ) {
		for ( %i = 0; %i < $StatHUD::PlayerCount[%team]-1; %i++ ) {
			for ( %j = %i+1; %j < $StatHUD::PlayerCount[%team]; %j++ ) {
				...
				if ( $Collector::Kills[ %namei ] < $Collector::Kills[ %namej ] ) {
...
$StatHUD::headerPixelSize[0] = 17 * $StatHUD::SpacerWidth;
...
function StatHUD::TabList( %item0, %item1, %item2, %item3, %item4, %item5, %item6, %item7, %item8, %item9 ) {
	%list = "\t\t\t\t";
...
function StatHUD::ShowStats() {
	...
	%topline = StatHUD::TabList( "Name", "Rating", "K/D", "Disc", "Mortar", "Chain", "CKill", "Idle" ) @ "\n";
	...
	for ( %team = -1; %team <= 1; %team++ ) {
		for ( %i = 0; %i < $StatHUD::PlayerCount[%team] ; %i++ ) {
			...
			%topline = %topline @ StatHUD::TabList(
				...
				StatHUD::KillDeathPair( $Collector::Kills[%name, "Mortar"], $Collector::Deaths[%name, "Mortar"] ),
				...
				//StatHUD::Number( $Collector::Kills[%name, "TeamKill"] ), 
				$StatHUD::IdleTime[%name]
			);
		}
